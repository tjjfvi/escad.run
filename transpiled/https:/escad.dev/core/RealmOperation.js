    ...
    create: (realm, operation) => {
        ...
        const that = Object.assign(new ExtensibleFunction((...args) => {
            ...
            if (Operation.isOperation(args[0])) {
                return RealmOperation.create(realm, Operation.create(operation.name + "+" + args[0].name, (a) => that(args[0](...a.val)), { overrideHierarchy: false }));
            }
            if (Component.isComponent(args[0])) {
                return RealmComponent.create(realm, Component.create(args[0].name + "+" + operation.name, (...a) => that(args[0](...a)), { overrideHierarchy: false }));
            }
