...
import { Hash } from "/transpiled/https://escad.dev/core/utils/mod.js";
...
import { ArrayProduct, ArrayProductType, HashProduct, HashProductType, MarkedProduct, MarkedProductType, TupleProduct, TupleProductType, UnknownProduct, UnknownProductType } from "/transpiled/https://escad.dev/core/product/mod.js";
export class ConversionRegistry {
  ...
  excludeStores = new Set([this.artifactStore]);
  registered = new Map();

  register(conversion) {
    this.registered.set(conversion.id, { ...conversion,
  ...
  maybeImplicitlyConvertibleTo(fromType, toType) {
    return false || Hash.equal(fromType, toType) || HashProductType.isHashProductType(fromType) || HashProductType.isHashProductType(toType) || UnknownProductType.isUnknownProductType(toType) || true && MarkedProductType.isMarkedProductType(fromType) && MarkedProductType.isMarkedProductType(toType) && fromType.marker === toType.marker || true && TupleProductType.isTupleProductType(fromType) && ArrayProductType.isArrayProductType(toType) || true && ArrayProductType.isArrayProductType(fromType) && ArrayProductType.isArrayProductType(toType) || true && TupleProductType.isTupleProductType(fromType) && TupleProductType.isTupleProductType(toType) && fromType.elementTypes.length === toType.elementTypes.length;
  }
  ...
  async finishPathSegment(fromType, toType) {
    ...
    if (MarkedProductType.isMarkedProductType(fromType) && MarkedProductType.isMarkedProductType(toType) && fromType.marker === toType.marker) {
    ...
    if (ArrayProductType.isArrayProductType(fromType) && ArrayProductType.isArrayProductType(toType)) {
    ...
    if (TupleProductType.isTupleProductType(fromType) && TupleProductType.isTupleProductType(toType) && fromType.elementTypes.length === toType.elementTypes.length) {
  ...
  async executeConversionPath(product, conversions) {
    ...
    return await this.artifactManager.computeRef([ConversionRegistry.artifactStoreId, conversions[conversions.length - 1].toType, product], async () => {
      let currentProduct = product;

      for (const conversion of conversions) {
        currentProduct = await artifactManager.computeRef([ConversionRegistry.artifactStoreId, conversion.toType, currentProduct], async () => conversion.convert(currentProduct), this.excludeStores);
      }
