...
export const Bsp = {
    ...
    mapExtra: (bsp, fn) => {
        ...
        for (const [origNode, origExtra] of nodeStack) {
            ...
            if (mappedExtras && mappedNode !== null) {
                nodeStack.push([mappedNode.back ?? null, mappedExtras[1]]).push([
            ...
            if (node.plane) {
                ...
                for (const face of faces) {
                    Plane.splitFace(node.plane, face, {
                        front: frontFaces,
                        back: backFaces,
                        coplanarFront: !!(options & ClipOptions.DropCoplanarFront) === dropFront
                            ? frontFaces
                            : backFaces,
                        coplanarBack: !!(options & ClipOptions.DropCoplanarBack) === dropFront
                            ? frontFaces
                            : backFaces,
                    });
                }
    ...
    clipTo: (a, b, options) => Bsp.map(a, (bsp) => bsp
        ? {
            ...bsp,
    ...
    build: (bsp, allFaces) => Bsp.mapExtra([
        bsp ?? Bsp.create(null, null, [], allFaces[0].plane),
        allFaces,
    ], (bsp, allFaces) => {
