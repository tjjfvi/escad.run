// @ts-nocheck
import { BufferGeometry, Float32BufferAttribute, Geometry } from "/transpiled/https://escad.dev/deps/three.js";
...
function EdgesGeometry(geometry, thresholdAngle = 1) {
  ...
  function runEdge(edge1, edge2, normal) {
    ...
    (map[edge2] = map[edge2] || []).push({
      o: edge1,
      n: normal
    });
  }

  this.setAttribute("position", new Float32BufferAttribute(keepEdges.concat(Object.entries(map).flatMap(([a, os]) => os.filter(o => !o.done).map(({
    o: b
  }) => [a, b]))).flatMap(e => e.flatMap(i => verts[i].toArray())), 3));
}
