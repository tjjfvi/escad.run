...
const epsilon = 1e-5;
const planeId = Id.create(import.meta.url, "@escad/3d", "LeafProduct", "Plane");
export const Plane = {
    id: planeId,
    create: _Plane,
    ...createLeafProductUtils(planeId, "Plane"),
    flip: (plane) => Plane.create(Vector3.negate(plane.normal), -plane.w),
    splitFace(plane, face, { front, back, coplanarFront, coplanarBack }) {
        ...
        switch (faceType) {
            case Coplanar:
                (Vector3.dot(plane.normal, face.plane.normal) > 0
                    ? coplanarFront
                    : coplanarBack).push(face);
                break;
            case Front:
                front.push(face);
                break;
            ...
            case Spanning: {
                ...
                face.points.map((vi, i, a) => {
                    ...
                    let t = (plane.w - Vector3.dot(plane.normal, vi)) /
                        Vector3.dot(plane.normal, Vector3.subtract(vj, vi));
