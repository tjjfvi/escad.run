...
import { _boundingBox } from "/transpiled/https://escad.dev/3d/chainables/getBoundingBox.js";
...
const moveToId = Id.create(import.meta.url, "@escad/3d", "Marker", "MoveTo");
export const MoveTo = MarkedProduct.for(moveToId);
const moveToArgsId = Id.create(import.meta.url, "@escad/3d", "LeafProduct", "MoveToArgs");
...
export const moveTo = Component.create("moveTo", (target, args) => {
  ...
  const targetBox = Element.isElement(target) ? _boundingBox(target) : BoundingBox.fromVector3(Vector3.create(target.x ?? 0, target.y ?? 0, target.z ?? 0));
  return mapOperation("moveTo", async (source, allSource) => Transform.create(TupleProduct.create([MoveTo.create(TupleProduct.create([MoveToArgs.create(args), await _boundingBox(allSource), await targetBox])), HashProduct.fromProduct(source)])), {
    showOutput: false
  });
}, {
  showOutput: false
});
export const moveToX = Component.create("moveToX", (target, toEdge = 0, shift = -Math.sign(toEdge)) => moveTo(target, {
  x: [toEdge, shift]
}), {
  showOutput: false
});
export const moveToY = Component.create("moveToY", (target, toEdge = 0, shift = -Math.sign(toEdge)) => moveTo(target, {
  y: [toEdge, shift]
}), {
  showOutput: false
});
export const moveToZ = Component.create("moveToZ", (target, toEdge = 0, shift = -Math.sign(toEdge)) => moveTo(target, {
  z: [toEdge, shift]
}), {
  showOutput: false
});
const moveToConversionMeshId = Id.create(import.meta.url, "@escad/3d", "Conversion", "MoveToConversionMesh");
conversionRegistry.register({
  ...
  convert: async ({
    child: {
      children: [args, sourceBox, targetBox]
    }
  }) => {
