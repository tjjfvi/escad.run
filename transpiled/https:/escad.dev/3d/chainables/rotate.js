...
export const rotate = Component.create("rotate", (...args) => {
  let [first] = args;
  let triple = args.length === 3 ? args : args.length === 4 ? args : typeof first === "object" ? first : [0, 0, 0];
  let opts = args.length === 4 ? args[3] : args.length === 2 ? args[1] : typeof first === "object" && "unit" in first ? first : undefined;
  ...
  return mapOperation("rotate", leaf => Transform.create(TupleProduct.create([m, leaf])), {
    showOutput: false
  });
}, {
  showOutput: false
});
export const rX = Component.create("rX", n => rotate({
  x: n
}), {
  showOutput: false
});
export const rY = Component.create("rY", n => rotate({
  y: n
}), {
  showOutput: false
});
export const rZ = Component.create("rZ", n => rotate({
  z: n
}), {
  showOutput: false
});
