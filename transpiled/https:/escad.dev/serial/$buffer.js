...
export const $buffer = new Serializer({
    *s(value) {
        ...
        yield* Serializer.write(value.byteLength, ({ buffer, offset }) => buffer.set(value, offset));
    },
    *d() {
        const length = yield* $uint32le.d();
        return yield* Serializer.read(length, ({ buffer, offset }) => new Uint8Array(buffer.buffer, buffer.byteOffset + offset, length));
    },
});
