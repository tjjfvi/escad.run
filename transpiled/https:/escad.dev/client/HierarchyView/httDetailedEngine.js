...
export const httDetailedEngine = {
    ...
    ValueHierarchy: ({ hierarchy }) => [TreePart.Line.String(hierarchy.value === null
            ? "null"
    ...
    CallHierarchy: ({ path, hierarchy, stateMemo, hierarchyToTree }) => {
        ...
        let curPath = path;
        let start = true;
        while (operands.length === 1 && CallHierarchy.isCallHierarchy(operands[0]) &&
        ...
        function _callHierarchyOperandsTree(path, operands) {
            return wrapTreeSelectable([...path, { type: "CallHierarchyPathPart", location: "allOperands" }], Hierarchy.flattenLinkedProducts(operands), [
                TreePart.Line.String("("),
                TreePart.Block({
                    ...
                    forceOpenable: operands.length !== 1 || !(ArrayHierarchy.isArrayHierarchy(operands[0]) ||
