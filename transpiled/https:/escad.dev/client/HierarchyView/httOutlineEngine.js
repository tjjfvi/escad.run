...
export const httOutlineEngine = {
  ...
  ArrayHierarchy: ({
    ...
    stateMemo,
    hierarchyToTree
  }) => httOutlineEngineArray(hierarchy.children.map((child, index) => hierarchyToTree({
    path: [...path, {
  ...
  ObjectHierarchy: ({
    ...
    children: Object.entries(hierarchy.children).map(([key, value]) => {
      ...
      const child = hierarchyToTree({
        hierarchy: value,
        path: newPath
      });
      return wrapTreeSelectable(newPath, value.linkedProducts, [TreePart.Line.String(key), ...(child.length ? [TreePart.Line.String(": "), ...child] : [])]);
    }),
    joiner: ", ",
  ...
  CallHierarchy: ({
    ...
    stateMemo,
    hierarchyToTree
  }) => hierarchy.composable ? httOutlineEngineArray(hierarchy.operands.map((child, index) => hierarchyToTree({
    path: [...path, {
