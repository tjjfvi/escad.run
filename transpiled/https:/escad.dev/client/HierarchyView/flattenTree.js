...
export function flattenTree(tree, maxLength, flattenOpenable = true) {
  while (true) {
    ...
    for (const part of tree) {
      const flattened = part.kind === "block" && !part.state.open && !part.forceEllipsis && (flattenOpenable || !part.forceOpenable) ? interleaveFlat(part.children, TreePart.Line.String(part.joiner ?? "")) : null; // no change
