...
import __balanced_match$ from "/transpiled/https://cdn.esm.sh/v64/balanced-match@1.0.2/deno/balanced-match.js";
import __concat_map$ from "/transpiled/https://cdn.esm.sh/v64/concat-map@0.0.1/deno/concat-map.js";
var G = Object.create;
...
var y = W((ur, Z) => { var Y = __concat_map$, x = __balanced_match$; Z.exports = rr; var E = "\0SLASH" + Math.random() + "\0", I = "\0OPEN" + Math.random() + "\0", g = "\0CLOSE" + Math.random() + "\0", N = "\0COMMA" + Math.random() + "\0", B = "\0PERIOD" + Math.random() + "\0"; function b(r) { return parseInt(r, 10) == r ? parseInt(r, 10) : r.charCodeAt(0); } function _(r) { return r.split("\\\\").join(E).split("\\{").join(I).split("\\}").join(g).split("\\,").join(N).split("\\.").join(B); } function k(r) { return r.split(E).join("\\").split(I).join("{").split(g).join("}").split(N).join(",").split(B).join("."); } function L(r) { if (!r)
    ...
    return t.post.match(/,.*\}/) ? (r = t.pre + "{" + t.body + g + t.post, p(r)) : [r]; var a; if (n)
    a = t.body.split(/\.\./);
    ...
    var M = b(a[0]), j = b(a[1]), D = Math.max(a[0].length, a[1].length), S = a.length == 3 ? Math.abs(b(a[2])) : 1, C = ar, H = j < M;
    H && (S *= -1, C = nr);
    var R = a.some(tr);
    f = [];
