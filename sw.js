...
sw.addEventListener("activate", event => {
  ...
  event.waitUntil(Promise.all([sw.clients.claim().then(async () => {
    for (let client of await sw.clients.matchAll({
      type: "window"
    })) {
...
sw.addEventListener("fetch", function (event) {
  ...
  async function handleCacheRequest(event) {
    ...
    if (isLocal && (path.startsWith("/artifacts/") || /^\/playground-[\w-]+\//.test(path))) {
    ...
    if (response.ok && response.type !== "basic") {
      ...
      caches.open(generalCache).then(cache => cache.put(event.request, cachedResponse));
    }
  ...
  async function handleApiRequest(event) {
    if (path === "/sw/clientId") {
      return new Response("playground-" + event.clientId, {
        status: 200
      });
    }
    ...
    if (cacheName !== "playground-" + event.clientId && !genericCaches.includes(cacheName)) {
